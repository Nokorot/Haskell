...
ballsFrame
  = do -- a list of balls, where each ball is represented
   ...
   where
      ...
      bouncing (Point x y)
        = point (bounce (x) 0 (-1)) (bounce (maxH-y) 0 (-1))
        -- = map (\h -> Point x (maxH-h)) (bounce (maxH-y) 0)

      point (x:xs) (y:ys) = (Point x (maxH-y)) : (point xs ys)
      point _ _    = []

      -- calculate all future heights
      bounce h v a
        | h <= 0 && v == 0 = replicate 20 radius -- keep still for 20 frames
        | h <= 0 && v  < 0 = bounce 0 ((-v)-2) a
        | otherwise        = h : bounce (h+v) (v+a) a
